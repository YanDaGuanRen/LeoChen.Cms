...
namespace Leochen.Cms.Results;

/// <summary>Csv动作结果</summary>
public class FormExtCsvResult: IActionResult
{
    /// <summary>
    /// 字段值转换委托，用于处理特定字段的值转换逻辑
    /// </summary>
    /// <param name="field">数据字段</param>
    /// <param name="entity">实体对象</param>
    /// <returns>转换后的值</returns>
    public delegate Object FieldValueConverter(DataField field, IModel entity);
    /// <summary>
    /// 字段值转换器
    /// </summary>
    public FieldValueConverter ValueConverter { get; init; } 
    ...
    public async Task ExecuteResultAsync(ActionContext context)
    {
        ...
        // 列头
        var headers = Fields.Select(e =>
        {
            if (!e.DisplayName.IsNullOrEmpty())
            {
        ...
        {
            await csv.WriteLineAsync(Fields.Select(e => ValueConverter(e, entity)));
        }
    }
}