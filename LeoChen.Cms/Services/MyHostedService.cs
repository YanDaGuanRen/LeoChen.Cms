...
namespace LenChen.Cms.Services;
...
public class MyHostedService(IRegistry registry, StarFactory factory) : IHostedService
{
    ...
    async Task DoGetInfo(Object state)
    {
        ...
        {
            // 从注册中心获取地址
            var services = await registry.ResolveAsync("LenChen.CmsApi");
            XTrace.WriteLine("LenChen.CmsApi服务信息：{0}", services.ToJson(true));

            // 创建指定服务的客户端，它的服务端地址绑定注册中心，自动更新
            _client = await factory.CreateForServiceAsync("LenChen.CmsApi") as ApiHttpClient;
            XTrace.WriteLine("LenChen.CmsApi服务地址：{0}", _client.Services.Select(e => e.Address).Join());
        }
