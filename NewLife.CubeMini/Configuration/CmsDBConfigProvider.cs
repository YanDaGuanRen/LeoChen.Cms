using System.Security.Cryptography;
using NewLife;
using NewLife.Configuration;
using NewLife.Cube.Entity;
using NewLife.Data;
using NewLife.Log;
using NewLife.Serialization;
using NewLife.Threading;
using XCode.Membership;

namespace NewLife.Cube.Configuration;

/// <summary>数据库参数表文件提供者，支持租户隔离</summary>
public class CmsDBConfigProvider : ConfigProvider
{
    #region 属性
    /// <summary>要加载配置的租户。默认0表示全局</summary>
    public Int32 TenantId => TenantContext.CurrentId;

    /// <summary>分类</summary>
    public String? Category { get; set; }

    /// <summary>本地缓存配置数据。即使网络断开，仍然能够加载使用本地数据，默认 Json</summary>
    public ConfigCacheLevel CacheLevel { get; set; } = ConfigCacheLevel.Json;

    /// <summary>更新周期。默认15秒，0秒表示不做自动更新</summary>
    public Int32 Period { get; set; } = 15;

    private IDictionary<String, Object?>? _cache;
    #endregion

    #region 方法
    /// <summary>初始化提供者，如有必要，此时加载缓存文件</summary>
    /// <param name="value"></param>
    public override void Init(String? value)
    {
        // 本地缓存。兼容旧版配置文件
        var name = Category;
        var path = NewLife.Setting.Current.DataPath;
        var file = path.CombinePath($"CmsDbConfig_{name}.json").GetFullPath();

        if ((Root == null || Root.Childs == null || Root.Childs.Count == 0) && CacheLevel > ConfigCacheLevel.NoCache && File.Exists(file))
        {
            XTrace.WriteLine("[{0}]加载缓存配置：{1}", Category, file);
            var txt = File.ReadAllText(file);

            // 加密存储
            if (CacheLevel == ConfigCacheLevel.Encrypted)
                txt = Aes.Create().Decrypt(txt.ToBase64(), name.GetBytes()).ToStr();

            txt = txt.Trim();
            var dic = txt.StartsWith("{") && txt.EndsWith("}") ? JsonParser.Decode(txt) : XmlParser.Decode(txt);
            if (dic != null) Root = Build(dic);
            // 如果位于分布式环境中，使用较短间隔，否则使用较长间隔
            if (Period == 15)
            {
                Period = Snowflake.GlobalWorkerId > 0 ? 15 : 60;
            }
        }
    }

    /// <summary>获取所有配置</summary>
    /// <returns></returns>
    protected virtual IDictionary<String, Object?> GetAll()
    {
        var dic = new Dictionary<String, Object?>(StringComparer.CurrentCultureIgnoreCase);

        // 减少日志
        using var showSql = CmsParameter.Meta.Session.Dal.Session.SetShowSql(false);

        // 读取所有配置项，不区分租户
        var list = CmsParameter.FindAllByCategory(Category);
        foreach (var item in list)
        {
            if (!item.Enable || item.Name.IsNullOrEmpty()) continue;

            if (item.TenantId > 0)
            {
                var key =  $"Tenant#{item.TenantId}#{item.Name}";
                dic[key] = item.Value;

                if (!item.Remark.IsNullOrEmpty())
                    dic["#" + key] = item.Remark;
            }
            else
            {
                dic[item.Name] = item.Value;
                if (!item.Remark.IsNullOrEmpty()) 
                    dic["#" + item.Name] = item.Remark;
            }
        }

        return dic;
    }

    /// <summary>加载配置字典为配置树</summary>
    /// <param name="configs"></param>
    /// <returns></returns>
    public virtual IConfigSection Build(IDictionary<String, Object?> configs)
    {
        // 换个对象，避免数组元素在多次加载后重叠
        var root = new ConfigSection { };
        foreach (var item in configs)
        {
            // 跳过备注
            if (item.Key[0] == '#') continue;

            var ks = item.Key.Split(':');
            var section = root;
            for (var i = 0; i < ks.Length; i++)
            {
                section = section.GetOrAddChild(ks[i]) as ConfigSection;
                if (section == null) break;
            }
            if (section == null) break;

            if (item.Value is IDictionary<String, Object?> dic)
                section.Childs = Build(dic).Childs;
            else
                section.Value = item.Value + "";

            // 加载备注
            if (configs.TryGetValue("#" + item.Key, out var remark))
                section.Comment = remark as String;
        }
        return root;
    }

    private Int32 _inited;

    /// <summary>加载配置</summary>
    public override Boolean LoadAll()
    {
        try
        {
            // 简化检查逻辑 - 只需确保初始化一次
            if (_inited == 0)
            {
                if (Interlocked.CompareExchange(ref _inited, 1, 0) == 0)
                {
                    Init(null);
                }
            }
        }
        catch { }

        try
        {
            IsNew = true;

            var dic = GetAll();
            if (dic != null && dic.Count > 0)
            {
                IsNew = false;

                Root = Build(dic);

                // 缓存
                SaveCache(dic);
            }
            else
            {
                // 首次加载，且配置文件存在，需要保存一份回去数据库。可能是从配置文件迁移为数据库配置
                if (Root != null && Root.Childs != null && Root.Childs.Count > 0)
                {
                    XTrace.WriteLine("[{0}]从文件保存配置到数据库", Category);
                    SaveAll();
                }
            }

            // 自动更新
            if (Period > 0) InitTimer();

            return true;
        }
        catch (Exception ex)
        {
            XTrace.WriteException(ex);

            return false;
        }
    }

    private void SaveCache(IDictionary<String, Object?> configs)
    {
        // 缓存
        _cache = configs;

        // 本地缓存
        if (CacheLevel > ConfigCacheLevel.NoCache)
        {
            var name = Category;
            var path = NewLife.Setting.Current.DataPath;
            var file = path.CombinePath($"CmsDbConfig_{name}.json").GetFullPath();
            if (System.IO.File.Exists(file))
            {
                var txt = configs.ToJson(true);

                // 加密存储
                if (CacheLevel == ConfigCacheLevel.Encrypted)
                    txt = Aes.Create().Encrypt(txt.GetBytes(), name.GetBytes()).ToBase64();

                File.WriteAllText(file.EnsureDirectory(true), txt);
            }

        }
    }

    /// <summary>保存配置树到数据源</summary>
    public override Boolean SaveAll()
    {
        var list = CmsParameter.FindAllByCategory(Category);
        Save(list, Root, null);

        // 通知绑定对象，配置数据有改变
        NotifyChange();

        return true;
    }

    void Save(IList<CmsParameter> list, IConfigSection root, String? prefix)
    {
        if (root == null || root.Childs == null) return;

        foreach (var section in root.Childs)
        {
            var name = prefix + section.Key;
            if (section.Childs != null && section.Childs.Count > 0)
            {
                Save(list, section, name + ":");
            }
            else
            {
                // 从配置键名中解析租户ID
                var (tenantId, actualName) = ParseTenantKey(name);
            
                var pi = list.FirstOrDefault(_ => _.Name == actualName && _.TenantId == tenantId);
                if (pi == null)
                {
                    pi = new CmsParameter { TenantId = tenantId, Category = Category, Name = actualName };
                    list.Add(pi);
                }

                pi.Value = section.Value;
                pi.Enable = true;

                if (!section.Comment.IsNullOrEmpty())
                    pi.Remark = section.Comment;

                pi.Save();
            }
        }
    }

    /// <summary>
    /// 从配置键名中解析租户ID和实际名称
    /// </summary>
    /// <param name="key">配置键名</param>
    /// <returns>(租户ID, 实际名称)</returns>
    private (Int32 tenantId, String name) ParseTenantKey(String key)
    {
        // 检查是否为租户特定配置键名格式: Tenant#{ID}#{Name}
        if (key.StartsWith("Tenant#") && key.Length > 7)
        {
            var parts = key.Split('#');
            if (parts.Length == 3 && Int32.TryParse(parts[1], out var tenantId))
            {
                return (tenantId, parts[2]);
            }
        }
    
        // 默认为全局配置
        return (0, key);
    }
    
    /// <summary>保存模型实例</summary>
    /// <typeparam name="T">模型</typeparam>
    /// <param name="model">模型实例</param>
    /// <param name="path">路径。配置树位置</param>
    public override Boolean Save<T>(T model, String? path = null)
    {
        if (model == null)
            throw new ArgumentNullException(nameof(model));
            
        // 直接调用 LoadAll 确保配置已加载
        LoadAll();
        
        // 获取或创建配置节点
        IConfigSection section = path.IsNullOrEmpty() ? Root : Find(path, true);
        if (section != null)
            section.MapFrom(model);
            
        return SaveAll();
    }
    
    /// <summary>获取或设置配置值，支持租户隔离</summary>
    /// <param name="key">键</param>
    public override String? this[String key]
    {
        get
        {
            LoadAll(); // 确保配置已加载
            return GetValueForTenant(key, TenantId);
        }
        set
        {
            LoadAll(); // 确保配置已加载
            SetValueForTenant(key, value, TenantId);
        }
    }
    #endregion

    #region 租户特定配置访问
    /// <summary>
    /// 为指定租户获取配置值
    /// </summary>
    /// <param name="key">配置键</param>
    /// <param name="tenantId">租户ID</param>
    /// <returns>配置值</returns>
    public String GetValueForTenant(String key, Int32 tenantId)
    {
        LoadAll(); // 确保配置已加载
        
        // 先尝试获取租户特定配置
        if (tenantId > 0)
        {
            var tenantKey = $"Tenant#{tenantId}#{key}";
            var section = Root?.Find(tenantKey, false);
            if (section != null) return section.Value;
        }
        
        // 回退到默认配置
        var defaultSection = Root?.Find(key, false);
        return defaultSection?.Value;
    }
    
    /// <summary>
    /// 为指定租户设置配置值
    /// </summary>
    /// <param name="key">配置键</param>
    /// <param name="value">配置值</param>
    /// <param name="tenantId">租户ID</param>
    /// <param name="forceGlobal">是否强制使用全局配置，为true时删除所有租户的特定配置</param>
    public void SetValueForTenant(String key, String value, Int32 tenantId, Boolean forceGlobal = false)
    {
        LoadAll(); // 确保配置已加载
        
        // 如果强制使用全局配置，删除所有租户的特定配置
        if (forceGlobal)
        {
            // 查找并删除所有租户的特定配置（以:key结尾的配置）
            if (Root != null && Root.Childs != null)
            {
                var sectionsToRemove = new List<IConfigSection>();
                
                foreach (var section in Root.Childs)
                {
                    if (section.Key != null && section.Key.StartsWith("Tenant#") && section.Key.EndsWith($"#{key}"))
                    {
                        sectionsToRemove.Add(section);
                    }
                }
                
                // 从配置树中移除租户特定配置
                foreach (var section in sectionsToRemove)
                {
                    Root.Childs.Remove(section);
                }
            }
            
            // 设置全局配置
            var globalSection = Root?.Find(key, true);
            if (globalSection != null)
            {
                globalSection.Value = value;
                SaveAll();
                // 更新缓存
                if (_cache != null) SaveCache(_cache);
            }
            return;
        }
        
        // 如果tenantId为0，直接保存全局配置
        if (tenantId == 0)
        {
            var globalSection = Root?.Find(key, true);
            if (globalSection != null)
            {
                globalSection.Value = value;
                SaveAll();
                // 更新缓存
                if (_cache != null) SaveCache(_cache);
            }
            return;
        }
        
        // 如果是租户特定配置，先检查是否与全局配置相同
        if (tenantId > 0)
        {
            // 获取全局配置值
            var globalSection = Root?.Find(key, false);
            var globalValue = globalSection?.Value;
            
            // 如果与全局配置相同，则不保存租户特定配置
            if (value == globalValue)
            {
                // 删除可能存在的租户特定配置
                var tenantKey = $"Tenant#{tenantId}#{key}";
                var tenantSection = Root?.Find(tenantKey, false);
                if (tenantSection != null)
                {
                    // 从配置树中移除租户特定配置
                    if (Root != null && Root.Childs != null)
                    {
                        Root.Childs.Remove(tenantSection);
                    }
                    SaveAll();
                    // 更新缓存
                    if (_cache != null) SaveCache(_cache);
                }
                return;
            }
        }
        
        var actualKey = tenantId > 0 ? $"Tenant#{tenantId}#{key}" : key;
        var section2 = Root?.Find(actualKey, true);
        if (section2 != null)
        {
            section2.Value = value;
            // 同步保存到数据库
            SaveAll();
            // 更新缓存
            if (_cache != null) SaveCache(_cache);
        }
    }
    #endregion

    #region 绑定
    /// <summary>绑定模型，使能热更新，配置存储数据改变时同步修改模型属性</summary>
    /// <typeparam name="T">模型</typeparam>
    /// <param name="model">模型实例</param>
    /// <param name="autoReload">是否自动更新。默认true</param>
    /// <param name="path">路径。配置树位置，配置中心等多对象混合使用时</param>
    public override void Bind<T>(T model, Boolean autoReload = true, String? path = null)
    {
        base.Bind<T>(model, autoReload, path);

        if (autoReload) InitTimer();
    }
    #endregion

    #region 定时
    /// <summary>定时器</summary>
    protected TimerX? _timer;
    private void InitTimer()
    {
        if (_timer != null) return;
        lock (this)
        {
            if (_timer != null) return;

            var p = Period;
            if (p <= 0) p = 60;
            _timer = new TimerX(DoRefresh, null, p * 1000, p * 1000) { Async = true };
        }
    }

    /// <summary>定时刷新配置</summary>
    /// <param name="state"></param>
    protected void DoRefresh(Object state)
    {
        using var showSql = CmsParameter.Meta.Session.Dal.Session.SetShowSql(false);

        var dic = GetAll();
        if (dic == null) return;

        var changed = new Dictionary<String, Object?>();
        if (_cache != null)
        {
            foreach (var item in dic)
            {
                // 跳过备注
                if (item.Key[0] == '#') continue;
                if (!_cache.TryGetValue(item.Key, out var v) || v + "" != item.Value + "")
                {
                    changed.Add(item.Key, item.Value);
                }
            }
        }

        if (changed.Count > 0)
        {
            XTrace.WriteLine("[{0}]配置改变，重新加载如下键：{1}", Category, changed.ToJson());

            Root = Build(dic);

            // 缓存
            SaveCache(dic);

            NotifyChange();
        }
    }
    #endregion
}
