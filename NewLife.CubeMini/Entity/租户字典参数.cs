...
/// <summary>租户字典参数。管理租户或系统全局的名值对数据，常用于参数配置场合</summary>
[Serializable]
[DataObject]
[Description("租户字典参数。管理租户或系统全局的名值对数据，常用于参数配置场合")]
[BindIndex("IU_CmsParameter_TenantId_Category_Name", true, "TenantId,Category,Name")]
[BindIndex("IX_CmsParameter_Category", false, "Category")]
[BindIndex("IX_CmsParameter_UpdateTime", false, "UpdateTime")]
[BindTable("CmsParameter", Description = "租户字典参数。管理租户或系统全局的名值对数据，常用于参数配置场合", ConnName = "Membership", DbType = DatabaseType.None)]
public partial class CmsParameter : IEntity<CmsParameterModel>
{
    #region 属性
    private Int32 _ID;
    /// <summary>编号</summary>
    ...
    /// <summary>租户。按租户区分参数，用户0表示系统级</summary>
    [DisplayName("租户")]
    [Description("租户。按租户区分参数，用户0表示系统级")]
    [DataObjectField(false, false, false, 0)]
    [BindColumn("TenantId", "租户。按租户区分参数，用户0表示系统级", "")]
    ...
    public void Copy(CmsParameterModel model)
    {
        ID = model.ID;
    ...
    public static CmsParameter FindByID(Int32 id)
    {
        if (id < 0) return null;
    ...
    /// <summary>根据租户、类别、名称查找</summary>
    /// <param name="tenantId">租户</param>
    /// <param name="category">类别</param>
    /// <param name="name">名称</param>
    /// <returns>实体对象</returns>
    public static CmsParameter FindByTenantIdAndCategoryAndName(Int32 tenantId, String category, String name)
    {
        ...
        // 实体缓存
        if (Meta.Session.Count < 1000) return Meta.Cache.Find(e => e.TenantId == tenantId && e.Category.EqualIgnoreCase(category) && e.Name.EqualIgnoreCase(name));

        return Find(_.TenantId == tenantId & _.Category == category & _.Name == name);
    }

    /// <summary>根据租户查找</summary>
    /// <param name="tenantId">租户</param>
    /// <returns>实体列表</returns>
    public static IList<CmsParameter> FindAllByTenantId(Int32 tenantId)
    {
    ...
    /// <summary>根据租户、类别查找</summary>
    /// <param name="tenantId">租户</param>
    /// <param name="category">类别</param>
    /// <returns>实体列表</returns>
    public static IList<CmsParameter> FindAllByTenantIdAndCategory(Int32 tenantId, String category)
    {
        if (tenantId < 0) return [];
        if (category.IsNullOrEmpty()) return [];

        // 实体缓存
        if (Meta.Session.Count < 1000) return Meta.Cache.FindAll(e => e.TenantId == tenantId && e.Category.EqualIgnoreCase(category));

        return FindAll(_.TenantId == tenantId & _.Category == category);
    }

    /// <summary>根据类别查找</summary>
    /// <param name="category">类别</param>
    /// <returns>实体列表</returns>
    public static IList<CmsParameter> FindAllByCategory(String category)
    {
        if (category.IsNullOrEmpty()) return [];
    ...
    /// <param name="tenantId">租户。按租户区分参数，用户0表示系统级</param>
    /// <param name="category">类别</param>
    ...
    /// <returns>实体列表</returns>
    public static IList<CmsParameter> Search(Int32 tenantId, String category, XCode.Membership.ParameterKinds kind, Boolean? enable, DateTime start, DateTime end, String key, PageParameter page)
    {
    ...
    #region 字段名
    /// <summary>取得租户字典参数字段信息的快捷方式</summary>
    ...
    /// <summary>取得租户字典参数字段名称的快捷方式</summary>
