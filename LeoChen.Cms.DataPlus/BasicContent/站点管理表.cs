...
[DataObject]
[Description("站点管理表")]
[BindIndex("IU_CmsSite_TenantId_AreaID", true, "TenantId,AreaID")]
[BindIndex("IX_CmsSite_TenantId", false, "TenantId")]
...
public partial class CmsSite : ICmsSite, IEntity<ICmsSite>
{
    ...
    /// <summary>根据租户、区域代码查找</summary>
    /// <param name="tenantId">租户</param>
    /// <param name="areaId">区域代码</param>
    /// <returns>实体对象</returns>
    public static CmsSite FindByTenantIdAndAreaID(Int32 tenantId, Int32 areaId)
    {
        if (tenantId < 0) return null;
        if (areaId < 0) return null;

        // 实体缓存
        if (Meta.Session.Count < 1000) return Meta.Cache.Find(e => e.TenantId == tenantId && e.AreaID == areaId);

        return Find(_.TenantId == tenantId & _.AreaID == areaId);
    }

    /// <summary>根据租户查找</summary>
    /// <param name="tenantId">租户</param>
    /// <returns>实体列表</returns>
    public static IList<CmsSite> FindAllByTenantId(Int32 tenantId)
    {
    ...
    /// <returns>实体列表</returns>
    public static IList<CmsSite> Search(Int32 tenantId, Int32 areaId, DateTime start, DateTime end, String key, PageParameter page)
    {
