...
[BindIndex("IU_CmsContent_TenantId_Title", true, "TenantId,Title")]
[BindIndex("IX_CmsContent_TenantId", false, "TenantId")]
...
public partial class CmsContent : ICmsContent, IEntity<ICmsContent>
{
    ...
    [BindColumn("Content", "内容", "LONGTEXT")]
    ...
    /// <summary>根据租户、标题查找</summary>
    /// <param name="tenantId">租户</param>
    /// <param name="title">标题</param>
    /// <returns>实体对象</returns>
    public static CmsContent FindByTenantIdAndTitle(Int32 tenantId, String title)
    {
        if (tenantId < 0) return null;
        if (title.IsNullOrEmpty()) return null;

        // 实体缓存
        if (Meta.Session.Count < 1000) return Meta.Cache.Find(e => e.TenantId == tenantId && e.Title.EqualIgnoreCase(title));

        return Find(_.TenantId == tenantId & _.Title == title);
    }

    /// <summary>根据租户查找</summary>
    /// <param name="tenantId">租户</param>
    /// <returns>实体列表</returns>
    public static IList<CmsContent> FindAllByTenantId(Int32 tenantId)
    {
    ...
    /// <returns>实体列表</returns>
    public static IList<CmsContent> Search(Int32 tenantId, Int32 areaId, Int32 contentSortId, Int32 contentSortSubId, String filename, Int32 sorting, Boolean? status, Boolean? isTop, Boolean? isRecommend, Boolean? isHeadline, DateTime start, DateTime end, String key, PageParameter page)
    {
